...
H_OFFSET = 10
H_DIST = 20
GREENS = cm.get_cmap('hsv', 8)
REDS = cm.get_cmap('hsv', 8)
...
for schd in SCHD_TYPES:
    for n in range(1, TOTAL_COUNT+1):
        ...
        ax.set_title(schd + '_' + str(n) + ' (total error = {} S)'.format(total_err))
        ax.set_xlabel('Time (second)')
        ax.xaxis.set_label_coords(0.5, -0.12)
        ax.set_ylabel('Process Names (Pretical downwards,theoretical upwards)')
